...
describe("runOnce", () => {
  ...
  it("fails an unconfigured routing job with a reason staff can read, rather than dropping it", async () => {
    const f = fakeStore({
      routing: {
        lead: { id: "lead-1", brand_id: BRAND, market_code: "EG" },
  ...
  it("logs the daily reconciliation at error level when a count that should be zero isn't", async () => {
    const logged: [string, string, Record<string, unknown> | undefined][] = [];
    const f = fakeStore({ report: { lead_dlq_open: 1, jobs_overdue: 0, events_24h: 40 } });
    f.jobs.push({
      id: "j-recon",
    ...
    await runOnce(f.store, { log: (level, event, fields) => logged.push([level, event, fields]) });

    const line = logged.find(([, event]) => event === "daily_reconciliation");
    expect(line?.[0]).toBe("error");
    expect(line?.[2]?.gaps).toEqual(["lead_dlq_open"]);
  });
});
