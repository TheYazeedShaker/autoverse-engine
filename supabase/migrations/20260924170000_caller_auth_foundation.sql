...
-- helpers. No function here is callable by anon or authenticated.
...
-- exactly what an Origin header carries. The last label must be alphabetic, so IP literals are out.
-- Plain http is allowed only for localhost, and only on a market that is not live.
...
returns boolean language sql immutable set search_path = '' as $fn$
  select coalesce(bool_and(
    o ~ '^https://[a-z0-9]([a-z0-9-]*[a-z0-9])?(\.[a-z0-9]([a-z0-9-]*[a-z0-9])?)*\.[a-z]{2,63}(:[0-9]{1,5})?$'
    or o ~ '^http://localhost(:[0-9]{1,5})?$'
  ), true)
  from unnest(coalesce(origins, '{}'::text[])) as o
$fn$;

create or replace function app_auth.any_localhost(origins text[])
returns boolean language sql immutable set search_path = '' as $fn$
  select coalesce(bool_or(o like 'http://localhost%'), false)
  from unnest(coalesce(origins, '{}'::text[])) as o
$fn$;
...
alter table public.brand_markets
  ...
  -- Any page on a visitor's own machine can claim to be localhost. A live market never trusts it.
  add constraint brand_markets_no_localhost_when_live check (not live or not app_auth.any_localhost(allowed_origins));
...
revoke all on public.brand_publishable_keys from anon, authenticated;
grant select on public.brand_publishable_keys to anon, authenticated;  -- RLS still decides which rows
...
-- Resolving a caller
-- ============================================================================================
-- The one place a public request becomes a brand. Returns the brand ONLY when the key is live, the
-- brand is live, the named market is live, and the origin is on THAT market's allowlist. Anything
-- else returns no row, and the caller learns nothing about which check failed.
--
-- The market is named by the caller (the page knows which market it is), never guessed. One site
-- can serve several markets of a brand, and guessing would route a Saudi lead to the Egyptian
-- team. Naming it grants nothing extra: the origin must still be on that market's own list.
--
-- Billing state is deliberately NOT checked. Data capture is never gated (CLAUDE.md); a brand
-- behind on payment still receives its leads.
create or replace function app_auth.resolve_public_caller(p_key text, p_origin text, p_market_code text)
...
as $fn$
  select bm.brand_id, bm.market_code
    ...
    join public.brand_markets bm  on bm.brand_id = k.brand_id and bm.market_code = p_market_code
   where k.key = p_key
...
create index rate_limit_hits_window_idx on app_auth.rate_limit_hits (window_start);
...
-- Counts this hit and says whether it is within the limit. A fixed window allows up to twice the
-- limit across a boundary, which is acceptable for abuse control. Windows are capped at an hour so
-- the cleanup (about 1 call in 100, anything older than a day) can never clear a live window.
-- Bucket naming is the caller's job (see PROGRESS.md): a keyed hash (HMAC) of the client, never a
-- plain hash of an IP, and a per-brand bucket too, so rotating IPs are still capped.
...
revoke execute on function app_auth.resolve_public_caller(text, text, text)       from public, anon, authenticated;
revoke execute on function app_auth.take_rate_limit(text, integer, integer) from public, anon, authenticated;
grant  execute on function app_auth.resolve_public_caller(text, text, text)       to service_role;
