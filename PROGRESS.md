...
1. **Continue `PAGE-CONSUMER-SHOWROOM`** (`specs/SPEC-page-consumer-showroom.md`), in a **local** session. The design source is the owner's copies in `design-approved/showroom/`, read with the file tools only, never the shell. One PR per slice against `main`; stop after each for the owner's merge. Anything the design shows that the schema lacks is Tier B, never invented. The EG consent value is HUMAN ONLY.
   ...
   - **Open: slice 2** (`feat/showroom-slice-2-cards`).
     ...
     - **Open Tier B:** the card surface and wedge, and whether icons use the accent. The approved file shows a white card with no wedge; spec §5.7 says greige + wedge, and no greige token exists. **Interim (code review H1): the card is on Mist**, with the inset panel set apart by a border. A brand accent is validated for AA only against Mist and white (brand_themes CHECKs), so any other surface could fail for some brand. The token test now asserts accent-on-Mist. The `SURFACE` constant in VehicleCard.tsx is the one place to change. Configure uses the accent per spec §2 + REV (architect, Tier A).
...
- `brand_themes.focus_hex` has only a format CHECK: nothing enforces 3:1 against Mist or white. validate-theme derives it with a 3:1 test, but the database does not guarantee it. Add the CHECK before the first real themed brand goes live (code review, slice 2).
