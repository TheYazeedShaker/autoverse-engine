...
| Rulesets on `main`                                                    | ✅ owner, 2026-09-25: `main-ci` (5 required checks, up to date, no bypass) and `main-review` (code-owner review, 0 approvals, no bypass). The classic rule is deleted; a direct push is rejected. ⚠ `.github/CODEOWNERS` contains the whole runbook (clean file sent to the owner), and the owner-PR auto-pass is unverified. See the runbook's open items. Auto-merge stays off until the merge-tiers ADR |
...
2. CODEOWNERS and the tiered auto-merge, plus an ADR for the merge tiers. CODEOWNERS is on `main` but needs cleaning (runbook open item 1). **The ADR must auto-merge only App-authored PRs that touch no code-owned path** (runbook open item 3).
