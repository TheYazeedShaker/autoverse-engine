...
- **`main` has no branch protection** (`404 Branch not protected`), so "no direct pushes to main, no overrides" is prose rather than a gate.
...
1. **Enable branch protection on `main`** — require a PR and passing checks. Until this exists the merge wall is prose. (Repo setting; `gh api` can do it with Yazeed's say-so.)
